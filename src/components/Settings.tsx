...
export default function Settings({ onClose }: SettingsProps) {
  ...
  const toggleCategory = (category: keyof typeof SITE_CATEGORIES) => {
    ...
    if (allAdded) {
      ...
      const newSites = categorySites.filter(
        (s) => !settings.blacklist.includes(s),
      );
      setSettings({
        ...settings,
            ...
            .map((t: any) => ({
              ...
              estimatedDuration:
                ...
                t.estimatedDuration > 0
                  ? Math.min(t.estimatedDuration, 1440)
                  : undefined,
              points:
                typeof t.points === "number"
                  ? Math.max(0, Math.min(t.points, 10000))
                  : undefined,
            }));
  ...
  const sections = [
    {
      id: "general" as const,
      icon: Monitor,
      label: t("tabGeneral", "General"),
    },
    {
      id: "blocking" as const,
      icon: Shield,
      label: t("tabBlocking", "Blocking"),
    },
  ...
  const categoryIcons: Record<string, { emoji: string; label: string }> = {
    ...
    entertainment: {
      emoji: "🎬",
      label: t("catEntertainment", "Entertainment"),
    },
    ...
    <div className="absolute inset-0 bg-white dark:bg-slate-900 flex flex-col z-50 animate-fade-in">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800">
        <h2 className="text-lg font-bold text-slate-900 dark:text-white">
          {chrome.i18n.getMessage("settingsHeader")}
        </h2>
        <button
          onClick={onClose}
          ...
          <>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-300 block">
                    {chrome.i18n.getMessage("settingsNotifications")}
                  </span>
                  <span className="text-[11px] text-slate-400">
                    {chrome.i18n.getMessage("settingsNotificationsDesc")}
                  </span>
                </div>
              </div>
              <Toggle
                ...
                onChange={(v) =>
                  setSettings({ ...settings, notificationsEnabled: v })
                }
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Toggle
                ...
                onChange={(v) =>
                  setSettings({ ...settings, gamificationEnabled: v })
                }
              />
            </div>
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">
                {t("feedbackSectionLabel", "Help Us Improve")}
              </label>
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    chrome.tabs.create({
                      url: "https://github.com/2gelbuy/ayamir/issues/new/choose",
                    })
                  }
                ...
                <button
                  onClick={() =>
                    chrome.tabs.create({
                      url: `https://chromewebstore.google.com/detail/ayamir/${chrome.runtime.id}/reviews`,
                    })
                  }
        ...
        {activeSection === "blocking" && (
          <>
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300 block">
                  {chrome.i18n.getMessage("settingsFocusMode")}
                </span>
                <span className="text-[11px] text-slate-400">
                  {chrome.i18n.getMessage("settingsFocusModeDesc")}
                </span>
              </div>
              <Toggle
                ...
                <div className="pl-2 space-y-2 animate-slide-down">
                  ...
                  <div className="flex gap-1">
                    {["M", "T", "W", "T", "F", "S", "S"].map((d, i) => {
                      ...
                      const isActive =
                        settings.scheduledBlocking.days.includes(dayNum);
                      return (
                        <button
                          key={i}
                          onClick={() => {
                            const days = isActive
                              ? settings.scheduledBlocking.days.filter(
                                  (x) => x !== dayNum,
                                )
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">
                {t("quickBlockCategories")}
              </label>
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(categoryIcons).map(
                  ([key, { emoji, label }]) => {
                    const sites =
                      SITE_CATEGORIES[key as keyof typeof SITE_CATEGORIES];
                    const allBlocked = sites.every((s) =>
                      settings.blacklist.includes(s),
                    );
                    const someBlocked = sites.some((s) =>
                      settings.blacklist.includes(s),
                    );
                    return (
                      <button
                        key={key}
                        onClick={() =>
                          toggleCategory(key as keyof typeof SITE_CATEGORIES)
                        }
                  ...
                  try {
                    ...
                    if (domains.length > 0) {
                      const newDomains = domains.filter(
                        (d) => !settings.blacklist.includes(d),
                      );
                      setSettings({
                        ...settings,
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">
                {t("settingsBlockedSites", "Blocked Sites")} (
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto slim-scrollbar">
                {settings.blacklist.map((domain) => (
                  <div
                    key={domain}
                    className="flex items-center justify-between py-1.5 px-3 bg-slate-50 dark:bg-slate-800 rounded-xl text-sm group"
                  >
          ...
          <>
              ...
              <div className="flex gap-2">
                {[15, 25, 45, 60, 90].map((d) => (
                  <button
                    key={d}
                    onClick={() =>
                      setSettings({ ...settings, deepWorkModeDuration: d })
                    }
              ...
              <div className="flex gap-2">
                {[3, 5, 10, 15].map((d) => (
                  <button
                    key={d}
                    onClick={() =>
                      setSettings({ ...settings, deepWorkModeBreakDuration: d })
                    }
                    className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
            ...
            <div className="flex items-center justify-between">
              ...
              <Toggle
                ...
                onChange={(v) => setSettings({ ...settings, hardLockMode: v })}
              />
            </div>
          </>
        )}
          ...
          <>
              ...
              {!showResetConfirm ? (
                ...
                <div className="p-3 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-200 dark:border-red-900/30 space-y-2">
                  ...
                  <p className="text-[11px] text-red-600/80 dark:text-red-500/70 leading-relaxed">
                    This will permanently delete all tasks, sessions, and
            ...
            <div className="text-center pt-4 space-y-1">
              <p className="text-xs text-slate-400">
                AyaMir v{chrome.runtime.getManifest().version}
              </p>
